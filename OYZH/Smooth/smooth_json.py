root_path = '/Users/mac/Downloads/PoseEstimation/'
...
'''
input: n * 2 dimension, num of keypoints * coordinates
'''
class Smooth_savgol_filter():
    ...
    def add_key_point(self, keypoints):
        ...
        # np.hstack((self.stack, xy_array))
        # self.stack.append(keypoints, axis=1)
        ...
        if self.stack.shape[1] > self.stack_len:
            ...
            for i in range(self.keypoints_dim):
                for j in range(2):
                    if self.stack[i + j * self.keypoints_dim][-1] == -1:
                        return_res[i][j] = -1
...
class Pose(object):
    ...
    kpt_names = ['nose', 'neck',
                 ...
                 'r_ear', 'l_ear']
    sigmas = np.array(
        [.26, .79, .79, .72, .62, .79, .72, .62, 1.07, 1.07, .87, .89, 1.07, .87, .89, .25, .25, .35, .35],
    ...
    def __init__(self, keypoints, confidence, kp_threshold=-0.0001):
        super().__init__()
        kp_info = np.array(keypoints).reshape(-1,
                                              3)  # keypoints的格式是[x_0, y_0, c_0, x_1, y_1, c_1, ...]，所以reshape成三个一行，前两点是坐标，后面一个点是score
    ...
    def draw_hrnet(self, img, ids=0):
        assert self.keypoints.shape == (Pose.num_kpts, 2)
        # Apply smooth filter
        self.keypoints = smooth_filter.add_key_point(self.keypoints)
