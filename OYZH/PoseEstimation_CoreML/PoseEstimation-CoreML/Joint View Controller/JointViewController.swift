...
//    typealias EstimationModel = model_cpm
    ...
    // Inference Result Data, for matching algorithm
    private var tableData: [PredictedPoint?] = []
    
    // Camera position
    private var cameraPos: Int = 0
    ...
    func switchCamera(){
        ...
        cameraPos += 1
        if (cameraPos %  2 == 1){
            videoCapture.setUp(sessionPreset: .vga640x480, cameraPosition: .front, isMirror: true) { success in
                if success {
                    // add preview view on the layer
                    if let previewLayer = self.videoCapture.previewLayer {
                        self.videoPreview.layer.addSublayer(previewLayer)
    ...
    func visionRequestDidComplete(request: VNRequest, error: Error?) {
        ...
        if let observations = request.results as? [VNCoreMLFeatureValueObservation],
            ...
            if predictedPoints.count != mvfilters.count {
                mvfilters = predictedPoints.map { _ in MovingAverageFilter(limit: 10) }
            }
